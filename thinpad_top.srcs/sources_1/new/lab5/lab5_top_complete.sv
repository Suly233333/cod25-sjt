/**
 * @file lab5_top.sv
 * @brief 五级流水线 CPU 顶层模块（完整版）
 *
 * 功能：
 * 1. PLL 时钟分频（50MHz -> 10MHz）
 * 2. 集成五级流水线各阶段模块（IF, ID, EXE, MEM, WB）
 * 3. Stall/Flush 控制器
 * 4. 寄存器文件（32个寄存器，32位宽）
 * 5. Wishbone 双端口仲裁器（IF 和 MEM）
 * 6. Wishbone 三路多路选择器（BaseRAM、ExtRAM、UART）
 * 7. SRAM 和 UART 控制器
 *
 * 地址映射：
 * - BaseRAM: 0x80000000 - 0x803FFFFF (4MB)
 * - ExtRAM:  0x80400000 - 0x807FFFFF (4MB)
 * - UART:    0x10000000 - 0x1000FFFF (64KB)
 */
...
module lab5_top (
  ...
  // ============================================================
  // WB 阶段 (隐含在 mem_wb_reg_r 中)
  // ============================================================
  // WB 阶段的功能是将 mem_wb_reg 中的数据写回寄存器文件
  // 这通过直接将 mem_wb_reg_r 的信号连接到寄存器文件的写端口来实现
